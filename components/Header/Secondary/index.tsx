...
export default function Header() {
  ...
  const menu = useMemo(
    () => [
      ...
      { name: t("features"), anchor: "#feature", id: "feature" },
      { name: t("tarif"), anchor: "#tarif", id: "tarif" },
  ...
  const resolveHref = (hash: string) => {
    ...
    if (isHome) return hash; // in-page anchor

    const prefix = currentLocale && currentLocale !== "en" ? `/${currentLocale}` : "";
    return `${prefix}/${hash}`; // e.g. /fr/#benefit
  };
  ...
  return (
    <Link
      ...
      className={`px-3 py-2 text-[15px] rounded-full font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/50 ${
        active
          ? sticky
            ? "text-darkBlue bg-blue-50"
            : "text-darkBlue bg-white/10"
          : ""
      } ${
        sticky
          ? "text-darkBlue hover:bg-slate-100"
          : "text-darkBlue hover:text-darkBlue hover:bg-slate-100"
      }`}
